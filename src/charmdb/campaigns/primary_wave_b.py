...
from charmdb.optimization.bo import primary_acquisition_name
...
from charmdb.paths import resolve_repository_reference
from charmdb.protocol import PRIMARY_SEARCH_METHODS, load_manifest
from charmdb.reporting.primary import METHOD_LABELS, render_primary_report
...
def _load_contract(
    ...
    primary_logical_path = Path(str(source["wave_a_manifest_path"]))
    primary_path = resolve_repository_reference(primary_logical_path)
    primary_sha, primary_payload = primary_manifest_payload(primary_path)
    if (
        primary_logical_path != Path("v2/config/primary-comparison.json")
        or primary_sha != source["wave_a_manifest_sha256"]
    ):
        raise ValueError("Wave B source Wave A manifest does not match D063")
    preregistration = resolve_repository_reference(Path(str(source["preregistration_path"])))
    if (
