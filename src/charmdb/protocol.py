...
from charmdb.paths import MANIFEST_ROOT, resolve_repository_reference
...
def load_manifest(path: Path) -> ProtocolManifest:
    path = resolve_repository_reference(path)
    try:
...
def validate_manifest_directory(directory: Path) -> list[ProtocolManifest]:
    directory = resolve_repository_reference(directory)
...
def main() -> int:
    ...
    parser.add_argument("directory", nargs="?", type=Path, default=MANIFEST_ROOT)
